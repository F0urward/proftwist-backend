...
func (h *ChatHandler) JoinGroupChat(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.chatUC.JoinGroupChat(ctx, chatID, userUUID); err != nil {
		...
		statusCode := http.StatusInternalServerError
		errorMsg := "failed to join group chat"

		if errs.IsForbiddenError(err) {
			statusCode = http.StatusForbidden
			errorMsg = "cannot join this chat"
		} else if errs.IsNotFoundError(err) {
			statusCode = http.StatusNotFound
			errorMsg = "chat not found"
		} else if errs.IsBusinessLogicError(err) {
			...
			statusCode = http.StatusConflict
			errorMsg = "already member of this chat"
		}
	...
	utils.JSONResponse(ctx, w, http.StatusOK, map[string]string{"message": "joined group chat successfully"})
}

func (h *ChatHandler) LeaveGroupChat(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.chatUC.LeaveGroupChat(ctx, chatID, userUUID); err != nil {
		...
		statusCode := http.StatusInternalServerError
		errorMsg := "failed to leave group chat"

		if errs.IsForbiddenError(err) {
			statusCode = http.StatusForbidden
			errorMsg = "cannot leave this chat"
		} else if errs.IsNotFoundError(err) {
			statusCode = http.StatusNotFound
			errorMsg = "chat not found"
		} else if errs.IsBusinessLogicError(err) {
	...
	utils.JSONResponse(ctx, w, http.StatusOK, map[string]string{"message": "left group chat successfully"})
}
