...
func (r *ChatHttpRegistrar) RegisterRoutes(s *httpServer.HttpServer) {
	s.MUX.Handle("/api/v1/chats/group/node/{node_id}", http.HandlerFunc(r.handlers.GetGroupChatByNode)).Methods("GET")
	...
	s.MUX.Handle("/api/v1/chats/group/{chat_id}/members", http.HandlerFunc(r.handlers.GetGroupChatMembers)).Methods("GET")
	s.MUX.Handle("/api/v1/chats/group/{chat_id}/messages", http.HandlerFunc(r.handlers.GetGroupChatMessages)).Methods("GET")
	...
	s.MUX.Handle("/api/v1/chats/direct/{chat_id}/member", s.AuthMiddleware.AuthMiddleware(http.HandlerFunc(r.handlers.GetDirectChatMembers))).Methods("GET")
