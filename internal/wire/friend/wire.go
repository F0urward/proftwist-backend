...
func InitializeFriendHttpServer(cfg *config.Config, log logger.Logger, mtrs metrics.Metrics) *httpServer.HttpServer {
	wire.Build(
		ClientsSet,
		FriendSet,
		AllHttpRegistrars,
...
func InitializeFriendGrpcServer(cfg *config.Config, log logger.Logger, mtrs metrics.Metrics) *grpcServer.GrpcServer {
	wire.Build(
		ClientsSet,
		FriendSet,
		AllGrpcRegistrars,
